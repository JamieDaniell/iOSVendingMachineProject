...
//  VendingMachine
//
//  Created by James Daniell on 15/05/2016.
...
class PlistConverter
{
    ...
    {
        guard let path = NSBundle.mainBundle().pathForResource(resource , ofType: type)
        else
        {
           throw InventoryError.InvalidResource
        }
        guard let dictionary = NSDictionary(contentsOfFile: path ), let castDictionary = dictionary as? [String: AnyObject]
        else
        {
    ...
    class func vendingInventoryFromDictionary(dictionary: [String: AnyObject]) throws -> [VendingSelection: ItemType]
    {
        ...
        for (key,value) in dictionary
        {
            if let itemDict = value as? [ String : Double ] , let price = itemDict["price"] , let quantity = itemDict["quantity"]
            {
                ...
                guard let key = VendingSelection(rawValue: key)
                else
                {
...
class VendingMachine: VendingMachineType
{
    ...
    func vend(selection: VendingSelection, quantity: Double) throws
    {
        guard var item = inventory[selection]
        else
        {
